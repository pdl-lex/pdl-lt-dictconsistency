    ...
    return rx.vstack(
        ...
        rx.cond(
            ...
            rx.vstack(
                ...
                results_grid(
                    ...
                    row_selection_handler=UniquenessState.set_selected_rows,
        ...
        rx.dialog.root(
            rx.dialog.content(
                rx.vstack(
                    rx.hstack(
                        rx.dialog.title(UniquenessState.preview_filename),
                        rx.spacer(),
                        rx.dialog.close(
                            rx.icon_button(rx.icon("x"), variant="ghost", on_click=UniquenessState.close_preview),
                        ),
                        width="100%",
                        align_items="center",
                    ),
                    rx.dialog.description("Zeile: ", UniquenessState.preview_line),
                    rx.box(
                        rx.html(UniquenessState.preview_content_with_line_numbers),
                        width="100%", height="500px", overflow_y="scroll", padding="10px",
                        background_color="var(--gray-2)", border="1px solid var(--gray-6)",
                        border_radius="4px", font_family="monospace", font_size="12px", line_height="1.5",
                    ),
                    rx.hstack(
                        rx.button("Schließen", on_click=UniquenessState.close_preview, variant="solid"),
                        width="100%", justify="end",
                    ),
                    spacing="3", width="100%",
                ),
                max_width="900px", width="90vw",
            ),
