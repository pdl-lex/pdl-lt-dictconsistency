...
// Zeichenweiser Diff: zeigt b mit gegenüber a geänderten Zeichen hervorgehoben.
export function Diff({ a, b }: { a: string; b: string }) {
  ...
  for (let i = 0; i < len; i++) {
    ...
    out.push(
      <span key={i} style={{
        background: same ? 'transparent' : 'var(--lt-primary-soft)',
        color: same ? 'inherit' : 'var(--lt-primary)',
        borderRadius: 2, padding: same ? 0 : '0 1px',
      }}>{ch}</span>,
    )
  }
  return <span style={{ fontFamily: 'var(--lt-font-mono)' }}>{out}</span>
}
