...
function RailGroup({ g, open, onToggle }: { g: MenuGroup; open: boolean; onToggle: () => void }) {
  const { activeId, setActiveId, setDataDialogOpen } = useWorkbench()
  const hasActive = g.items.some((it) => it.id === activeId)
  const handle = (id: string) => { if (id === 'data') setDataDialogOpen(true); else setActiveId(id) }
  return (
          ...
          {g.items.map((it) => {
            ...
            return (
              <button key={it.id} disabled={it.disabled}
                onClick={() => !it.disabled && handle(it.id)}
                style={{
                  ...railRow(active), padding: '5px 8px 5px 34px', fontSize: 12.5,
                  background: active ? 'var(--lt-primary-soft)' : 'transparent',
                  color: it.disabled ? 'var(--lt-fg-4)' : active ? 'var(--lt-primary)' : 'var(--lt-fg-2)',
                  fontWeight: active ? 600 : 400, cursor: it.disabled ? 'default' : 'pointer',
                }}>
                {it.label}
              </button>
            )
